...
// Called every frame
void UDynamicTextureComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UDynamicTextureComponent::DrawCircle(int32 StartX, int32 StartY, int32 Size, FLinearColor Color,
                                          bool Center /*= true*/)
{
	float radius = Size / 2;
...
void UDynamicTextureComponent::DrawFromTexture(int32 StartX, int32 StartY, UTexture2D* Texture,
                                               FLinearColor Filter /*= FLinearColor::White*/)
{
	if (!Texture)
	{
		return;
	...
	for (int32 y = 0; y < height; y++)
	{
		for (int32 x = 0; x < width; x++)
		{
			...
			SetPixelValue(StartX + x, StartY + y, FColor((texData[start + 2] * Filter.R), texData[start + 1] * Filter.G,
			                                             texData[start] * Filter.B, texData[start + 3] * Filter.A));
		}
	}
